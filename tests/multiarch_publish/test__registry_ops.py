...
class RegistryOpsTests(unittest.TestCase):
    ...
    def test_inspect_raw_manifest_raises_command_error_on_invalid_json(self) -> None:
        with patch(
            "multiarch_publish._registry_ops.run_command", return_value="not-json"
        ):
    ...
    def test_resolve_platform_manifest_digest_raises_when_no_candidate_matches(
        self,
    ) -> None:
        entry = PlatformDigest(
            platform=Platform(os="linux", architecture="arm", variant="v7"),
    ...
    def test_resolve_attestation_digest_returns_attestation_manifest_digest(
        self,
    ) -> None:
        manifest = {
            "manifests": [
    ...
    def test_resolve_platform_verification_digests_raises_when_attestation_missing(
        self,
    ) -> None:
        entry = PlatformDigest(
            platform=Platform(os="linux", architecture="arm64"),
    ...
    def test_verify_attestation_contains_provenance_accepts_slsa_predicate(
        self,
    ) -> None:
        manifest = {
            "layers": [
        ...
        with patch(
            ...
            _verify_attestation_contains_provenance(
                "ghcr.io/acme/test", "sha256:attestation"
            )

    def test_verify_attestation_contains_provenance_raises_without_provenance(
        self,
    ) -> None:
        manifest = {
            "layers": [
                {
                    "annotations": {
                        "in-toto.io/predicate-type": "https://example.com/other"
                    }
                },
                {"annotations": {}},
            ]
    ...
    def test_publish_manifest_by_digest_returns_digest(self) -> None:
        ...
        command_strings = [
            " ".join(call.args[0]) for call in run_command_mock.call_args_list
        ]
        self.assertIn(
    ...
    def test_publish_manifest_by_digest_raises_when_digest_lookup_is_empty(
        self,
    ) -> None:
        entries = [
            PlatformDigest(Platform(os="linux", architecture="amd64"), "sha256:amd64"),
    ...
    def test_run_verify_command_retries_no_signatures_found(self) -> None:
        with patch(
            "multiarch_publish._registry_ops.run_command",
            side_effect=[
                CommandError("command failed: cosign verify\nno signatures found"),
                "",
            ],
        ) as run_command_mock, patch(
            "multiarch_publish._registry_ops.time.sleep"
        ) as sleep_mock:
            _run_verify_command(
                ["cosign", "verify", "ghcr.io/acme/test@sha256:manifest"]
            )
    ...
    def test_sign_and_verify_platform_image_checks_attestation(self) -> None:
        ...
        self.assertIn(
            "cosign sign --yes ghcr.io/acme/test@sha256:index", command_strings
        )
        self.assertIn(
            "cosign sign --yes ghcr.io/acme/test@sha256:platform", command_strings
        )
        self.assertIn(
    ...
    def test_sign_and_verify_manifest_verifies_target_digest(self) -> None:
        with patch(
            "multiarch_publish._registry_ops.run_command", return_value=""
        ) as run_command_mock:
            sign_and_verify_manifest(
                image_ref="ghcr.io/acme/test",
        ...
        self.assertTrue(
            any(
                "cosign sign --yes ghcr.io/acme/test@sha256:manifest" == text
                for text in command_strings
            )
        )
